...
[X] (CDX) Step 20: Add Playwright web-runner package skeleton with actions (click/fill/select/wait/assert/extract) and basic selector ladder resolution

[X] (CDX) Step 21: Add orchestrator web client wrapper (in-proc library) and a minimal `system.ping` equivalent for web-runner
...
[X] (CDX) Step 24: Create first sample workflow in `workflows/samples/` (simple web-only) and a Playwright smoke test

[O] (CDX) Step 25: Create second sample workflow (desktop-only) that focuses a window and fills a text box with verification asserts
...
[X] (CDX) Step 29: Implement synthesizer v0 candidate generation (web + desktop) from recordings

[X] (CDX) Step 30: Implement synthesizer v0 evaluation metrics: coverage, uniqueness, stability, cost

[O] (CDX) Step 31: Implement disambiguation loop (web: scoping, role/name; desktop: label proximity, container signature)

[O] (CDX) Step 32: Implement greedy minimization of selectors (remove predicates while keeping coverage + uniqueness)

[X] (CDX) Step 33: Output compiled `selectors.json` and `assertions.json` into workflow package format

[X] (CDX) Step 34: Add verification mode in orchestrator: Explain → Act → Verify per step, always capturing evidence

[O] (CDX) Step 35: Build verifier UI package skeleton (simple local web UI or Electron shell) to show step details and the 3 decision buttons

[O] (CDX) Step 36: Implement decision handling: Proceed, Proceed with Comments, Raise Issue with Comments
...
[X] (CDX) Step 40: Add CI workflow (`.github/workflows/ci.yml`) for lint + unit tests + web smoke tests
...
[X] (CDX) Step 47: Establish test frameworks and conventions
...
[X] (CDX) Step 48: Add contract/schema validation tests
...
[O] (CDX) Step 49: Add integration test harness (headless + mocked desktop)
...
[O] (CDX) Step 51: Docs generation pipeline
...
[O] (CDX) Step 52: GitHub Actions workflows expansion
...
[O] (CDX) Step 54: Add sample workflow “verification run” golden artifacts
